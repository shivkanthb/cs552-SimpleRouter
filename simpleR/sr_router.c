...
void handle_ip_packet(struct sr_instance* sr, uint8_t* packet, unsigned int len, struct sr_if* rx_if, struct sr_ethernet_hdr* rx_e_hdr)
{
    ...
    if (chk_ip_addr(rx_ip_hdr, sr) == NULL)
    {
...
void forward_packet(struct sr_instance* sr, uint8_t* packet, unsigned int len)
{
    ...
    while(temp != NULL)
    {
        ...
        else
        {
            if (route == NULL)
            {
                if ((temp->dest.s_addr & temp->mask.s_addr) == (rx_ip_hdr->ip_dst.s_addr & temp->mask.s_addr))
                {
                    route = temp;
                }
            }
    ...
    if (tx_interface != NULL)
    {
        ...
        if (item == NULL)
        {
            ...
            if (route != NULL)
            {
                send_arp_request(sr, tx_interface, route->gw.s_addr);
            }
            else if (default_route != NULL)
            {
...
struct sr_if* chk_ip_addr(struct ip* rx_ip_hdr, struct sr_instance* sr)
{
